...
/**
 * 类描述:     [Lesson03Array 实战题目 移动零  盛水最多  爬楼梯  两数之和  三数之和  ]
 * 创建人:     [jkdong]
 * 创建时间:   [2020/11/06 22:01:34]
 * 版本:       [v1.0]
 */

class SolutionLesson03Array {
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        // minIdx 是最小指针
        for (int minIdx = 0; minIdx < len - 2 ; minIdx++) {
            //  先处理特殊情况
            //  a + b + c = 0 至少有一个负数和至少有一个正数 排序后如果最小指针的元素大于0就没必要比较了
            if(nums[minIdx] > 0){
                break;
            }
            //  最小指针元素存在重复则略过
            ...
            //  初始化左右指针
            int leftIdx = minIdx + 1, rightIdx = len -1;
            while (leftIdx < rightIdx){
                int sum = nums[minIdx] + nums[leftIdx] + nums[rightIdx];

                if(sum < 0){
                    //  和小于0 左指针右移 如果元素相同则略过
                    while(leftIdx < rightIdx && nums[leftIdx] == nums[++leftIdx]);
                }else if(sum > 0){
                    //  和大于0 右指针左移 如果元素相同则略过
                   while (leftIdx < rightIdx && nums[rightIdx] == nums[--rightIdx]);
                }else {
                    res.add(new ArrayList<Integer>(Arrays.asList(nums[minIdx],nums[leftIdx],nums[rightIdx])));
                    while (leftIdx < rightIdx && nums[leftIdx] == nums[++leftIdx]);
                    while (leftIdx < rightIdx && nums[rightIdx] == nums[--rightIdx]);
                }
            }
